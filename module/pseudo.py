...
class UsernameOSINT(JustradamusModule):
                ...
                if not_found_texts:
                    ...
                    for text in not_found_texts:
                        if text.lower() in content:
                            console.print(f"    {site_name} : non trouvé")
                            self.not_found.append(site_name)
                            return
                console.print(f"[+] {site_name:<15} : {url}")
                self.found.append((site_name, url))
                return
            console.print(f"    {site_name} : non trouvé")
            self.not_found.append(site_name)
        except Exception:
            console.print(f"[!] {site_name} : Timeout/Block")
    ...
    def run_scan(self):
        console.print("[>] USERNAME OSINT SCANNER")
        console.print(f"Pseudo cible : {self.username}")
        console.print("-" * 40)
        ...
        console.print("-" * 40)
        console.print(f"[+] Profils trouvés : {len(self.found)}")
        console.print(f"    Non trouvés     : {len(self.not_found)}")
        console.print(f"[!] Erreurs         : {len(self.errors)}")
        console.print(f"    Total testé     : {total}")
        console.print(f"    Rapport         : {path}")
        console.print("-" * 40)

        if self.found:
            console.print("[>] LIENS DIRECTS")
            for site, url in self.found[:10]:
                console.print(f"[+] {site:<15} : {url}")
            if len(self.found) > 10:
                console.print(f"    ... et {len(self.found) - 10} autres résultats")